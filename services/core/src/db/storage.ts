 ...
 *  - Embedded PGlite (WASM postgres, file-persisted) otherwise — zero-config demo
...
const PG_LITE_DATA_DIR = process.env.PGLITE_DIR ?? ".chalo-data";
...
export async function openStorage(): Promise<Storage> {
  ...
  const db = new PGlite(PG_LITE_DATA_DIR);
  return {
    kind: "pglite",
    sql: {
      async query<T>(text: string, params?: unknown[]): Promise<QueryResult<T>> {
